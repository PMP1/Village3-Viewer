...
// Interior fillers keep the timber edge contained in the plain wall source;
// exterior fillers receive their structural posts as separate overlays.
const PNG_INTERIOR_WALL_1_LEFT = Object.freeze({...PNG_EXTERIOR_FRONT_WALL_1,"sourceX":16});
const PNG_INTERIOR_WALL_1_RIGHT = Object.freeze({...PNG_EXTERIOR_FRONT_WALL_1,"sourceX":48});
...
const BUILDING_PNG_SPRITES = Object.freeze({
    ...
    "building.exterior.back.wall1.left": PNG_EXTERIOR_FRONT_WALL_1,
    "building.exterior.back.wall1.right": PNG_EXTERIOR_FRONT_WALL_1,
    ...
    "building.interior.wall.horizontal2.plain": PNG_EXTERIOR_FRONT_WALL_2,
...
const BUILDING_PNG_PATHS = Object.freeze({
    "building.exterior.back.wall2.plain": EXTERIOR_FRONT_WALL_PATH,
    "building.exterior.back.wall1.left": EXTERIOR_FRONT_WALL_PATH,
    "building.exterior.back.wall1.right": EXTERIOR_FRONT_WALL_PATH,
    ...
    "building.interior.wall.horizontal2.plain": EXTERIOR_FRONT_WALL_PATH,
    "building.interior.wall.horizontal1.left": EXTERIOR_FRONT_WALL_PATH,
    "building.interior.wall.horizontal1.right": EXTERIOR_FRONT_WALL_PATH,
...
function horizontalPanel(segment, layer, face) {
    return segment.layer === layer && segment.orientation === "horizontal" &&
        (face === undefined || segment.face === face) &&
        (segment.role === "wall-bay-2" || segment.role === "wall-filler-1");
}

function horizontalRuns(segments, layer, face) {
    const groups = new Map();
    for (const segment of segments) {
        if (segment.layer !== layer || segment.orientation !== "horizontal" ||
            (face !== undefined && segment.face !== face)) continue;
        const key = segment.side + ":" + segment.y;
    ...
    for (const group of groups.values()) {
        ...
        for (const segment of ordered) {
            if (run.length > 0 && segment.x > end) {
                runs.push(run);
                run = [];
                end = undefined;
            }
            run.push(segment);
            end = Math.max(end ?? segment.x, segment.x + (segment.length ?? 1));
        }
...
function sharedHorizontalPostSegments(segments, layer, face, role, stoneCorners) {
    const posts = [];
    for (const run of horizontalRuns(segments, layer, face)) {
        const runStart = run[0].x;
        const runEnd = Math.max(...run.map(segment => segment.x + (segment.length ?? 1)));
        for (let index = 0; index < run.length; index++) {
            const segment = run[index];
            if (!horizontalPanel(segment, layer, face)) continue;
            ...
            if (!before || horizontalPanel(before, layer, face)) {
                posts.push({
                    ...segment, x: segment.x, length: 0, role,
                    postKind: stoneCorners && segment.x === runStart ? "corner" : "timber"
                });
            }
            if (!after) {
                const x = segment.x + (segment.length ?? 1);
                posts.push({
                    ...segment, x, length: 0, role,
                    postKind: stoneCorners && x === runEnd ? "corner" : "timber"
                });
            }
        }
    }
...
function sharedRearPostSegments(segments) {
    return sharedHorizontalPostSegments(segments, "exterior", "rear", "shared-rear-post", true);
}

function sharedFrontPostSegments(segments) {
    return sharedHorizontalPostSegments(segments, "exterior", "front", "shared-front-post", true);
}

function sharedInteriorPostSegments(segments) {
...
window.VillageBuildingAtlas = Object.freeze({
    ...
    rearPostSource: EXTERIOR_FRONT_TIMBER_POST_PATH,
    rearPostSprite: PNG_EXTERIOR_FRONT_POST,
